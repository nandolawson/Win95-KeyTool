...
/*Code*/
fn main() {
	let args: Vec<String> = env::args().collect(); // Get arguments from terminal
	...
	let product_key: String = args[1].to_string();
	if product_key == "generate" {println!("Windows 95 key: {}", generate_product_key())}
	else if validate_product_key(product_key.clone()) {println!("Valid key: {}", product_key)}
